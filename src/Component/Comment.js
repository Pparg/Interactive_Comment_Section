...
export function Comment({data_comment, user, handleReply, id, deleteComment} ) {
    ...
    return(<>
        <section className="comment_container">
            ...
            <div className="main_content">
                ...
                <div className="comment_section">
                    {edit?<div className="update"><textarea value={reply} onChange={(e)=> setReply(e.target.value)}></textarea><button>Update</button></div> :<h2>{data_comment.content}</h2>}
                </div>
            </div>
        </section>
        {clicked? 
        ...
        <div className="reply_comment">
            {data_comment.replies.map((replie,index)=><Reply key={index} id={replie.id} info={replie} current={user} deleteComment={deleteComment} handleReply={handleReply} parentId={id}/>)}
        </div>
        </>
    )
